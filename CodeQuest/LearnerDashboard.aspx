<%@ Page Language="C#" %>

<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml" lang="en">
<head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>Learner Dashboard | CodeQuest</title>
</head>
<body style="margin:0;min-height:100vh;display:grid;place-items:center;color:#f5f9ff;background:#07101d;font-family:Arial,sans-serif;">
    <main style="max-width:620px;padding:40px;text-align:center;">
        <h1 style="margin:0 0 12px;color:#4fe4ff;">Learner login successful</h1>
        <p style="color:#93a8bf;line-height:1.6;">This temporary page confirms that the learner session and redirect work. Build the learner dashboard here next.</p>
        <a href="Login.aspx" style="color:#4fe4ff;">Return to login</a>
    </main>
</body>
</html>
